...
$.fn.dateSelect = function(options) {
    ...
    var tz_y, tz_m, tz_d, tz_y_s, tz_y_e,wf_year,wf_month,wf_day;
    ...
    $("#datepicker_" + elem_id +" .day").on("click","li",function(){
        ...
        var select_day = tz_y+"/"+tz_m+"/"+tz_d;
        showWeekFirstDay(select_day);

        (wf_month < 10 ) ? tz_m_str = "0" + wf_month : tz_m_str = wf_month;

        (wf_day < 10 ) ? tz_d_str = "0" + wf_day : tz_d_str = wf_day;

        var date_sel = wf_year + "-" + tz_m_str + "-" + tz_d_str;
    ...
    function dayListReload()
    {
        ...
        var select_day = new Date(tz_y+"/"+tz_m+"/"+tz_d).getDay();
        var select_date= tz_y+"/"+tz_m+"/"+tz_d;
        showWeekFirstDay(select_date); //当前选择时间所在周的周一日期
        console.log(wf_year,wf_month,wf_day);
        ...
        for(var i=1;i<=days;i++)
        {
            ...
            for(j=0;j<=6;j++){
                var day=wf_day+j;
                if(tz_y === input_y && tz_m === input_m && day === i)
                {
                    a_str = "active";
                }
            }
    ...
    function showWeekFirstDay(time)
    {
        ...
        var res = Nowdate-(Nowdate.getDay()-1)*86400000;
        console.log(res);
        if(Nowdate.getDay()===0){
